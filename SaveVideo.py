...
def main():
    ...
    for icam in range(1, 9):
        ...
        for current_frame in range(startFrame, endFrame):
            ...
            img_traj = cv2.copyMakeBorder(img_traj, 10, 10, 0, 10, cv2.BORDER_CONSTANT, value=[255, 255, 255])

            img_heat = cal_heatmap(icam, current_frame, startFrame, find_ind_heat)
            img_heat = cv2.resize(img_heat, (640, 360))
            img_heat = cv2.copyMakeBorder(img_heat, 0, 10, 10, 10, cv2.BORDER_CONSTANT, value=[255, 255, 255])
